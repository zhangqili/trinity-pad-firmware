...
{
    current_target_key_binding = &g_keymap[layer_select_menu.selected_index][key_select_list.list.selected_index];
...
void keymappage_init()
{
    ...
    fezui_animated_listbox_init(&key_select_list, key_select_list_items, sizeof(key_select_list_items) / sizeof(const char *), key_select_list_cb);
...
static void keymappage_logic(void *page)
{
    ...
    fezui_animated_listbox_update(&fezui, &key_select_list);
...
static void show_binding_text()
{
    for (uint8_t i = 0; i < LAYER_NUM; i++)
    {
        keyid_prase(g_keymap[i][key_select_list.list.selected_index], binding_text[i], 128);
