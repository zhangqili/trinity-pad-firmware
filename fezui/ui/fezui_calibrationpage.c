...
static void calibrate_upper_bound()
{
    ...
    {
        g_keyboard.advanced_keys[i].config.upper_bound = g_keyboard.advanced_keys[i].raw;
        max_values[i] = g_keyboard.advanced_keys[i].raw;
        min_values[i] = g_keyboard.advanced_keys[i].raw;
    }
}
...
static void calibrate_lower_bound()
{
    for (int i = 0; i < ADVANCED_KEY_NUM; i++)
    {
        if (fabsf(g_keyboard.advanced_keys[i].config.upper_bound - max_values[i]) > fabsf(g_keyboard.advanced_keys[i].config.upper_bound - min_values[i]))
        {
            g_keyboard.advanced_keys[i].config.lower_bound = max_values[i];
            g_keyboard.advanced_keys[i].config.calibration_mode = ADVANCED_KEY_AUTO_CALIBRATION_POSITIVE;
        }
        else
        {
            g_keyboard.advanced_keys[i].config.lower_bound = min_values[i];
            g_keyboard.advanced_keys[i].config.calibration_mode = ADVANCED_KEY_AUTO_CALIBRATION_NEGATIVE;
        }
    }
...
static void drawrawdata(uint8_t x, uint8_t y, uint8_t n)
{
    ...
    fezui_printf_right_aligned(&fezui, WIDTH / 2 - MARGIN_RIGHT, MARGIN_TOP + ROW_HEIGHT * 1, "%s", g_keyboard.advanced_keys[n].key.state ? "True" : "False");

    fezui_printf(&fezui, WIDTH / 2 + MARGIN_LEFT, MARGIN_TOP + ROW_HEIGHT * 1, "mode");
    fezui_printf_right_aligned(&fezui, WIDTH - MARGIN_RIGHT, MARGIN_TOP + ROW_HEIGHT * 1, "%d", g_keyboard.advanced_keys[n].config.mode);

    fezui_printf(&fezui, MARGIN_LEFT, MARGIN_TOP + ROW_HEIGHT * 2, "raw");
    fezui_printf_right_aligned(&fezui, WIDTH / 2 - MARGIN_RIGHT, MARGIN_TOP + ROW_HEIGHT * 2, "%4.3f", g_keyboard.advanced_keys[n].raw);

    fezui_printf(&fezui, WIDTH / 2 + MARGIN_LEFT, MARGIN_TOP + ROW_HEIGHT * 2, "value");
    fezui_printf_right_aligned(&fezui, WIDTH - MARGIN_RIGHT, MARGIN_TOP + ROW_HEIGHT * 2, "%.3f", g_keyboard.advanced_keys[n].value);

    fezui_printf(&fezui, MARGIN_LEFT, MARGIN_TOP + ROW_HEIGHT * 3, "distance");
    fezui_printf_right_aligned(&fezui, WIDTH - MARGIN_RIGHT, MARGIN_TOP + ROW_HEIGHT * 3, "(%.3f, %.3f)", g_keyboard.advanced_keys[n].config.trigger_distance, g_keyboard.advanced_keys[n].config.release_distance);

    fezui_printf(&fezui, MARGIN_LEFT, MARGIN_TOP + ROW_HEIGHT * 4, "speed");
    fezui_printf_right_aligned(&fezui, WIDTH - MARGIN_RIGHT, MARGIN_TOP + ROW_HEIGHT * 4, "(%.3f, %.3f)", g_keyboard.advanced_keys[n].config.trigger_speed, g_keyboard.advanced_keys[n].config.release_speed);

    fezui_printf(&fezui, MARGIN_LEFT, MARGIN_TOP + ROW_HEIGHT * 5, "deadzone");
    fezui_printf_right_aligned(&fezui, WIDTH - MARGIN_RIGHT, MARGIN_TOP + ROW_HEIGHT * 5, "(%.3f, %.3f)", g_keyboard.advanced_keys[n].config.upper_deadzone, g_keyboard.advanced_keys[n].config.lower_deadzone);

    fezui_printf(&fezui, MARGIN_LEFT, MARGIN_TOP + ROW_HEIGHT * 6, "bound");
    fezui_printf_right_aligned(&fezui, WIDTH - MARGIN_RIGHT, MARGIN_TOP + ROW_HEIGHT * 6, "(%.3f, %.3f)", g_keyboard.advanced_keys[n].config.upper_bound, g_keyboard.advanced_keys[n].config.lower_bound);

    fezui_printf(&fezui, MARGIN_LEFT, MARGIN_TOP + ROW_HEIGHT * ROW_HEIGHT, "calibration mode");
    switch (g_keyboard.advanced_keys[n].config.calibration_mode)
    {
...
static void calibrationpage_tick(void *page)
{
    ...
    case CALIBRATION_PRESS:
        for (int i = 0; i < ADVANCED_KEY_NUM; i++)
        {
            if (g_keyboard.advanced_keys[i].raw > max_values[i])
            {
                max_values[i] = g_keyboard.advanced_keys[i].raw;
            }
            if (g_keyboard.advanced_keys[i].raw < min_values[i])
            {
                min_values[i] = g_keyboard.advanced_keys[i].raw;
            }
        }
